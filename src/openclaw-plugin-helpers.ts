...
function parseAkkLifecycleCommand(
  ...
  if (action === "repair-model-control") {
    const usage = "Usage: /akk repair-model-control <exact-terminal-id>";
    const { token: terminalId, rest: extra } = takeRequiredToken(rest, usage);
    assertExactTerminalId(terminalId, usage);
    if (extra.trim()) {
      throw new Error(usage);
    }
    return { action: "repair-model-control", terminalId };
  }
  if (action === "set-model") {
    const usage =
 ...
 *
 * The CLI remains the complete operator/debug contract.  The controller receives a
 * terminal-first projection: current resource identities and state, action
 * names, and only the dynamic semantic inputs needed to invoke those actions.
 * Static explanations live in the bundled agent-knock-knock skill.
 */
...
function compactAkkListManaged(value: unknown): Record<string, unknown> | undefined {
  ...
  const history = arrayValue(managed.history)
    .map((turn) => compactAkkListTurn(turn));
...
function compactAkkListActionInput(
  ...
  const args = compactAkkListActionArguments(
    action.arguments,
    name,
    parentIds
  );
...
function compactAkkListSemanticValue(
  value: unknown,
  depth: number
): unknown {
  if (
...
export function formatAkkModelOptionsCommandResult(
  ...
  return [
    ...
    "This successful catalog result has restored an exact empty Composer and carries a fresh ordinary private offer for one set-model attempt in the current controller. Residual-entry authority was consumed by model-options and is never reusable by set-model. A Codex rollout and identify-foreground are not prerequisites. Refresh /akk models after any terminal change or failed attempt."
  ].join("\n");
}
...
export function formatAkkRepairModelControlCommandResult(
  result: Record<string, unknown>
): string {
  const outcome = nonEmptyString(result.outcome) ?? "uncertain";
  ...
  return [
    repaired
      ? "AKK cleared and verified the exact stale Codex /model surface."
      : "AKK could not prove the model-control repair postcondition.",
    `terminal: ${nonEmptyString(result.terminal_id) ?? "unknown"}`,
    `composer postcondition: ${nonEmptyString(result.composer_postcondition) ?? "unproven"}`,
    `terminal input attempted: ${result.terminal_input_attempted === true ? "yes" : "no"}`,
    ...compatibilityWarningLines(result),
    ...(nonEmptyString(result.reason)
      ? [
          `reason: ${sanitizeAkkModelFacingDiagnosticText(
      ...
      : []),
    uncertain
      ? "Next: do not retry automatically; inspect the shared pane and refresh /akk list."
      : "Next: refresh /akk list, then run /akk models before any deliberate set-model attempt.",
    "This repair never submits a task, chooses a model, approves a prompt, or creates an AKK Turn."
  ].join("\n");
}

export function isAkkRepairModelControlSuccess(result: unknown): boolean {
  const record = recordValue(result);
  return record?.outcome === "repaired" || record?.outcome === "already_empty";
}
