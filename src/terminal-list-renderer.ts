...
export function listActionContracts(): JsonRecord {
  return {
    ...
    instructions: [
      ...
      "Availability is a snapshot. terminal_user_explicit revalidates the exact live process and provider-owned terminal identity, approval state, and current composer against the new request; AKK recovery state never vetoes it. Other actions revalidate their action-specific workspace, activity, management, and recovery authority before side effects."
    ],
    field_semantics: {
      process_state: {
    ...
    actions: {
      send: {
        ...
        initial_attach_scope:
          "A terminal_user_explicit structured call uses the terminal_id prefilled by an exact live terminal row's available send action, or omits the target only when exactly one send-ready pane exists. Human slash commands may preserve a discovery selector explicitly named by the user. This user-priority scope depends only on the exact live physical prompt, no active approval, and an agent-supported composer. Codex submits an identical existing draft with Enter only, clears and replaces a different existing draft, or injects into an empty composer; Claude Code remains exact-empty-only. If the managed fast path cannot deliver before input, the physical operation runs once as unmanaged work with no callback Turn or callback, then best-effort releases that management. Refresh list afterward and use Watch to observe the task.",
        ...
        ordinary_use:
          "Create a new managed Turn through the exact action listed for the pane when the managed fast path succeeds. An explicit session_id never follows the pane and is unavailable for rollout-backed Codex Sessions; their listed follow-current action binds only the unique exact rollout that accepts the submitted request. The managed path can attach an unmanaged pane, adopt one verified human-selected native context, detach a verified-empty Codex source, or replace an eligible status-card/candidate-rollout source. A terminal_user_explicit action gives the user's selected exact live physical prompt priority over broken AKK internal state: Codex gives an identical draft Enter only, clears and replaces a different draft, or injects into an empty composer; Claude Code remains exact-empty-only. The unmanaged path has no callback Turn or callback and best-effort releases conflicting management afterward. Refresh list and use Watch for that still-running task."
      },
      retry_submission: {
        tool: "agent_knock_knock_send",
