    ...
    actions: {
      send: {
        ...
        initial_attach_scope:
          "A terminal_user_explicit structured call uses the terminal_id prefilled by an exact live terminal row's available send action, or omits the target only when exactly one send-ready pane exists. Human slash commands may preserve a discovery selector explicitly named by the user. This user-priority scope depends only on the exact live physical terminal/process and a scanned, non-blocked approval state. Codex physical fallback clears the current Composer once with C-u, injects the request, waits through the paste window, and dispatches Enter exactly once without a post-text Composer veto; Claude Code remains exact-empty-only. The managed fast path may require exact empty before input, but user-explicit Codex delivery never uses post-text Composer observation to veto Enter. If that managed path cannot deliver before input, the physical operation runs once as unmanaged work with no managed callback Turn; AKK then best-effort attaches an exact Terminal Watch callback and releases stale management. Watch attachment failure never changes a successful Send.",
        ...
        ordinary_use:
          "Create a new managed Turn through the exact action listed for the pane when the managed fast path succeeds. An explicit session_id never follows the pane and is unavailable for rollout-backed Codex Sessions; their listed follow-current action binds only the unique exact rollout that accepts the submitted request. The managed path can attach an unmanaged pane, adopt one verified human-selected native context, detach a verified-empty Codex source, or replace an eligible status-card/candidate-rollout source. A terminal_user_explicit action gives the user's selected exact live physical terminal/process priority over broken AKK internal state: Codex physical fallback clears the current Composer once, injects the request, waits through the paste window, and dispatches Enter exactly once without any post-text Composer veto; Claude Code remains exact-empty-only. The unmanaged path has no managed callback Turn; it best-effort attaches an exact Terminal Watch callback and releases conflicting management afterward. Watch attachment failure is a warning and never revokes delivery."
      },
      retry_submission: {
        tool: "agent_knock_knock_send",
