...
export function registerOpenClawCommands(
  ...
  api.registerCommand?.({
    ...
    agentPromptGuidance: [
      "Use /akk <task> when exactly one send-ready coding-agent terminal pane should receive new work. Send-ready means an exact live process and terminal, a verified non-blocked approval state, and an agent-supported composer; parsed working activity does not veto this user-priority path. Codex terminal_user_explicit Send accepts an empty composer, submits an identical draft with Enter only, or clears and replaces a different stable draft before Enter; Claude Code user-explicit Send remains exact-empty-only. Managed Send, native inspection, and native lifecycle input also remain exact-empty-only. Broken or stale AKK management activity records do not veto the user's physical Send. Structured tools use only semantic identifiers returned by AKK: session_id for an exact managed context, terminal_id for the currently verified pane, turn_id for one managed Turn, watch_id for one Terminal Watch, and native_thread_id for one resumable native thread. Draft text, composer digests, and opaque freshness authority stay private; AKK revalidates them under its locks. A failure after the sole Codex draft-clear key is uncertain and must not be automatically retried. Use /akk watch only from a currently advertised watch action to observe one human-started active task; it sends no input and creates no Session or Turn. New/clear/resume, approval, reconciliation, handoff, and recovery still require the documented user intent or explicit confirmation. AKK never starts a coding-agent process."
    ],
