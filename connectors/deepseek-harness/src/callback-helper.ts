...
import { sendCallbackOverIpc } from "./ipc.js";
...
/** Execute the Profile callback helper without exposing IPC authority in argv. */
export async function runCallbackHelper(options: {
  ...
  const response = await sendCallbackOverIpc({ socketPath, request });
  return {
    request: {
      delivery_id: options.arguments.deliveryId,
      message_id: options.arguments.messageId,
    },
    result: {
...
export function parseCallbackHelperArguments(argv: readonly string[]): CallbackHelperArguments {
  ...
  for (let index = 0; index < argv.length; index += 2) {
    ...
    if (!flag || !supported.has(flag) || !value || values.has(flag)) {
      throw new Error("invalid AKK callback helper arguments");
    }
...
async function main(): Promise<void> {
  ...
  try {
    ...
    process.stderr.write("agent-knock-knock callback helper configuration is invalid\n");
    process.exitCode = 2;
  }
}
...
async function readStandardInput(): Promise<string> {
  ...
  for await (const chunk of process.stdin) {
    ...
    bytes += buffer.length;
    if (bytes > CALLBACK_MAX_REQUEST_BYTES) {
      throw new Error("AKK callback body exceeded its limit");
    }
    chunks.push(buffer);
  }
