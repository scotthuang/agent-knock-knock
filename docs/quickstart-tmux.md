...
AKK Turn status and native Codex status are separate. `/akk status` and `agent_knock_knock_status` inspect AKK-managed state and a bounded current screen; they never type `/status`. For a native status card, start from `/akk list` and use only an advertised `native_inspect` action through `agent_knock_knock_native_inspect`, preserving its exact `terminal_id`, `inspection="status"`, and `expected_binding_token`. Initial support is Codex-only and limited to versions 0.146.0/0.146.1. The action creates no Session, Turn, receipt, monitor, or callback. Claude native commands, `/usage`, `/model`, `/compact`, and arbitrary slash commands remain unavailable. Bare Codex `/usage` is not read-only automation: it opens a menu whose later Enter can select an account-side usage-limit reset.
