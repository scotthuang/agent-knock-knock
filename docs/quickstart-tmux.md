...
If Status instead returns a pending native questionnaire `interaction_state`, use `agent_knock_knock_respond_interaction` only from that same controller conversation and only with the current projection's `turn_id`, `interaction_id`, semantic question/option IDs, or bounded typed text. It answers one current step. Refresh Status before each later question or final confirmation; leave `manual_required`, changed, expired, secret-bearing, or uncertain prompts for direct terminal handling.
