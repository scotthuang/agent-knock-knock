...
| `test/terminal-list-renderer.test.ts`, `test/openclaw-plugin-helpers.test.ts`, `test/quickstart-docs.test.ts` | Fresh `available_actions.watch`, Watch status/unwatch routing and formatting, action-contract v19 semantic-ID-only projection, and the documented TUI → fresh list → Watch workflow |
