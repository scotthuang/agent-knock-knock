...
- `AKK: <task>`: call `agent_knock_knock_send` with `request=<task>` and neither target ID. This succeeds only when exactly one send-ready pane exists; parsed activity and broken AKK management state do not veto an otherwise exact empty physical prompt.
