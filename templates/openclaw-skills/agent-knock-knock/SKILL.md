...
For an exact managed Turn, Status may also return a native questionnaire `interaction_state`. Treat it as a single-use, current-step projection, not as ordinary `waiting_for_openclaw` response text. The private response offer exists only in the same controller conversation that displayed that Status result. Require `state="pending"` and `capabilities.respond=true`, review the projected question with the user, and pass only its semantic ids and typed answer to `agent_knock_knock_respond_interaction`. Refresh Status after every successful step because the next question or final confirmation has a new `interaction_id`. A `manual_required`, expired, changed, secret, or uncertain interaction must remain for human inspection; do not send raw keys or retry blindly.
