...
- `AKK: <task>`: call `agent_knock_knock_send` with `request=<task>` and neither target ID. This succeeds only when exactly one send-ready pane exists. A scanned blocked approval observation vetoes physical Send; parsed working activity and Codex Composer observation do not. Codex replaces the current Composer with the request and submits exactly once; Claude Code remains exact-empty-only. Broken or stale AKK management activity records do not veto that independently verified live terminal/process.
...
- An answer to a native coding-agent questionnaire is a different flow: in the same controller conversation, call `agent_knock_knock_status({turn_id})`, show the current pending `interaction_state` to the user, and require their explicit choice or text. Only when `capabilities.respond=true`, call `agent_knock_knock_respond_interaction` with that projection's exact `turn_id`, `interaction_id`, and typed `answers` using its opaque `question_id` and `option_id` values. One call answers only the current step; call Status again before each later question or final confirmation. Never guess ids, translate a label into terminal keys, answer a `manual_required` or secret-input state, or retry an uncertain response blindly.
...
Approval attention is notification-only in either manual Watch mode. An automatic exact request-bound Watch created by `terminal_user_explicit` unmanaged fallback can additionally notify questionnaire attention after exact request acceptance. Show the user that the watched TUI needs attention and tell them to inspect and decide there. Never call an approve or interaction-response tool for a Watch, send approval/questionnaire keys or text, or apply `autoApprove`; each new exact attention fingerprint is notified once while the Watch remains active. Terminal outcomes settle once. The durable outbox uses deterministic notification IDs/idempotency and leased retry, so startup and periodic supervision can safely recover callback delivery after AKK, OpenClaw, or Gateway restart.

The current plugin registers 17 OpenClaw tools and list action-contract v24. Every structured model action carries semantic IDs only; opaque fences, Composer digests, and draft text are derived or retained privately. Watch uses `agent_knock_knock_watch({terminal_id})`, `agent_knock_knock_status({watch_id})`, and `agent_knock_knock_unwatch({watch_id})`; its internal CLI boundary is `watch-terminal`, `watch-status`, `unwatch-terminal`, and `reconcile-watches`.
