...
import { ensureStoreWritable, loadState, saveState } from "../src/store.js";
...
test("deferred management abandonment is a neutral released acceptance outcome", async (t) => {
  const root = fs.mkdtempSync(path.join(
    os.tmpdir(),
    "akk-acceptance-user-abandoned-"
  ));
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  const storeDir = path.join(root, "store");
  const conversationDir = path.join(storeDir, "conversations", "turn-release");
  ...
  const conversation = {
    ...createConversation({
      userRequest: "keep the coding task running",
    ...
    native_session_takeover: {
      ...
      deferred_foreground_transfer_id: "transfer-release"
    }
  };
  ...
  const facade = createTerminalAcceptanceCliFacade({
    deferred: {
      loadTransfer: () => {
        ...
        return { transfer_id: "transfer-release", status };
      },
      recover: async () => {
        events.push("recover");
      },
      loadAuthority: () => {
        throw new Error("released acceptance must not load native authority");
      },
      assertLedgerAuthority: () => {
        throw new Error("released acceptance must not inspect the dispatch ledger");
      }
    }
  } as unknown as TerminalAcceptanceCliDependencies);
  const bridge = {
    resolveStoredTerminal: async () => {
      ...
      throw new Error("released acceptance must not resolve the native process");
    },
    proveExactDraftStillPresent: async () => {
      events.push("probe-composer");
      throw new Error("released acceptance must not probe the composer");
    }
  } as unknown as TerminalAcceptanceBridge;
  const before = fs.readFileSync(statePath, "utf8");

  for (status of ["user_abandoning", "user_abandoned"] as const) {
    events.length = 0;
...
test("concurrent close after deferred acceptance reload remains neutral", async (t) => {
  const root = fs.mkdtempSync(path.join(
    os.tmpdir(),
    "akk-acceptance-concurrent-close-"
  ));
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  const storeDir = path.join(root, "store");
  const conversationDir = path.join(storeDir, "conversations", "turn-race");
  const statePath = path.join(conversationDir, "state.json");
  const logPath = path.join(conversationDir, "events.ndjson");
  const control: TerminalControlRef = {
    kind: "tmux",
  ...
  const conversation = {
    ...createConversation({
      userRequest: "race close with acceptance",
    ...
    native_session_takeover: {
      terminal_bridge: true,
      terminal_agent_pid: 42,
      deferred_foreground_transfer_id: "transfer-race"
    }
  };
  saveState(statePath, conversation);
  let status: "dispatch_started" | "user_abandoning" = "dispatch_started";
  let closedState = "";
  const events: string[] = [];
  const facade = createTerminalAcceptanceCliFacade({
    deferred: {
      loadTransfer: () => {
        events.push(`load:${status}`);
        return { transfer_id: "transfer-race", status };
      },
      recover: async () => {
        events.push("recover");
        ...
        closedState = fs.readFileSync(statePath, "utf8");
      },
      loadAuthority: () => {
        throw new Error("abandonment race must not load accepted authority");
      },
      assertLedgerAuthority: () => {
        throw new Error("abandonment race must not write accepted authority");
      }
    }
  } as unknown as TerminalAcceptanceCliDependencies);
  const bridge = {
    resolveStoredTerminal: async () => {
      events.push("resolve-native");
      return { terminalControl: control };
    },
    proveExactDraftStillPresent: async () => {
      events.push("probe-composer");
      throw new Error("abandonment race must not probe the composer");
    }
  } as unknown as TerminalAcceptanceBridge;
  ...
  assert.deepEqual(events, [
    "load:dispatch_started",
    "resolve-native",
    "recover",
    "load:user_abandoning"
  ]);
  assert.equal(fs.readFileSync(statePath, "utf8"), closedState);
  assert.equal(fs.existsSync(logPath), false);

  const deferred = sourceBetween(
    "async #reconcileDeferredAcceptance(",
    "#acceptedDeferredAuthority("
  );
  assertOrdered(deferred, [
    "loadTransfer(storeDir, transferId)",
...
test("mark uncertain reload fences abandonment before every durable write", async (t) => {
  for (const abandonmentStatus of [
    "user_abandoning",
    "user_abandoned"
  ] as const) {
    await t.test(abandonmentStatus, async () => {
      const root = fs.mkdtempSync(path.join(
        os.tmpdir(),
        `akk-mark-uncertain-${abandonmentStatus}-`
      ));
      try {
        const storeDir = path.join(root, "store");
        ...
        const control: TerminalControlRef = {
          kind: "tmux",
          target: "uncertain-race:0.0",
          session: "uncertain-race",
          window: 0,
          pane: 0,
          panePid: 42,
          capabilities: []
        };
        const requestText = "do not mark after explicit close intent";
        const messageId = "message-uncertain-race";
        const conversation = {
          ...createConversation({
            userRequest: requestText,
            sessionId: "session-uncertain-race",
            turnId: "turn-uncertain-race",
            executorKind: "codex"
          }),
          status: "waiting_for_agent" as const,
          store_dir: storeDir,
          ...
          native_session_takeover: {
            ...
            deferred_foreground_transfer_id: "transfer-uncertain-race",
            terminal_bridge_submission: {
              status: "text_injected",
              session_id: "session-uncertain-race",
              turn_id: "turn-uncertain-race",
              message_id: messageId,
              request_hash: terminalBridgeRequestFingerprint(requestText),
              prepared_at: "2026-08-23T00:00:00.000Z",
              text_injected_at: "2026-08-23T00:00:01.000Z"
            }
          }
        };
        saveState(statePath, conversation);
        const before = fs.readFileSync(statePath, "utf8");
        let expectedAfterBarrier = before;
        let transferStatus: "dispatch_started" | typeof abandonmentStatus =
          "dispatch_started";
        let ledgerWrites = 0;
        let transferLoads = 0;
        const facade = createTerminalAcceptanceCliFacade({
          repository: {
            acquireStateLock() {
              ...
              transferStatus = abandonmentStatus;
              if (abandonmentStatus === "user_abandoned") {
                saveState(statePath, {
                  ...conversation,
                  status: "closed",
                  closed_at: "2026-08-23T00:00:01.500Z",
                  close_reason: "closed by request",
                  disposition: "user_abandoned_management",
                  callback_expected: false
                });
                expectedAfterBarrier = fs.readFileSync(statePath, "utf8");
              }
              return () => undefined;
            },
            loadLedger() {
              ledgerWrites += 1;
              throw new Error("abandonment must fence uncertain ledger reads");
            },
            saveLedger() {
              ledgerWrites += 1;
              throw new Error("abandonment must fence uncertain ledger writes");
            }
          },
          deferred: {
            loadTransfer() {
              transferLoads += 1;
              return {
                transfer_id: "transfer-uncertain-race",
        ...
        await runCliCommandExecution("mark-uncertain-close-race", {}, {
          now: () => new Date("2026-08-23T00:00:02.000Z")
        }, async () => {
          const result = facade.markUncertain({
            conversation,
            statePath,
            logPath,
            terminalControl: control,
          ...
          assert.equal(
            result.status,
            abandonmentStatus === "user_abandoned"
              ? "closed"
              : "waiting_for_agent"
          );
        });
        assert.equal(
          transferLoads,
          abandonmentStatus === "user_abandoned" ? 0 : 1
        );
        assert.equal(ledgerWrites, 0);
        assert.equal(fs.readFileSync(statePath, "utf8"), expectedAfterBarrier);
        assert.equal(fs.existsSync(logPath), false);
        assert.equal(
          fs.existsSync(path.join(storeDir, ".akk-writer.lock")),
          false
