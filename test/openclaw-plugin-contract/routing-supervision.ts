  ...
  try {
    ...
    assert.match(
      isRecord(terminalIdSchema)
        ...
        : "",
      /Codex and Claude Code terminal_user_explicit[\s\S]*exact live physical terminal\/process[\s\S]*scanned, non-blocked approval state[\s\S]*Composer visibility, stability, exactness[\s\S]*not eligibility vetoes[\s\S]*C-u[\s\S]*Claude Code[\s\S]*sentinel-backed native C-s stash-clear transaction[\s\S]*cursor position[\s\S]*proves? the main Composer empty[\s\S]*paste window[\s\S]*Enter exactly once[\s\S]*without a post-text Composer veto[\s\S]*Broken AKK state cannot veto[\s\S]*no managed callback Turn[\s\S]*Terminal Watch callback/u
    );
